...
\begin{document}
	...
	\begin{exampleblock}{BOLD response model}
		\[
		B(t) =  \tikzmark{first}\mathcal{H}(\tikzmark{second}r(t, \Theta=\tikzmark{third}\theta)) + \tikzmark{forth}e(t)
		\]
		\begin{tikzpicture}[
		remember picture,
		overlay,
		...
		\draw<2->[arrow]
		(firstN) to[out=90,in=180] ([yshift=0.5ex]{pic cs:first});  
	...
	\begin{itemize}
		...
		\item
		The population response of such neurons in general can $\boldsymbol{not}$ be modeled using a model that sums contrast linearly across the visual field. hence, Compressive spatial summation (CSS) model is used,
	\end{itemize}
	...
	\draw<2->[arrow]
	(firstN2) to[out=90,in=180] ([yshift=0.5ex]{pic cs:first2});  

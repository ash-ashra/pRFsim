...
def generateData(neuronal_responses, hrf, noise):
  ...
  for i in range(sqrtVoxels): # do with multithreading
      for j in range(sqrtVoxels):
          ...
          bolds[:, i, j] = np.convolve(hrf, n)[0:duration] + norm.rvs(scale=noise, size=duration)
  return bolds
...
# def fun_der(x, bold, hrf):
#     global radius
#     global precision
#     global stim
#     return Jacobian(lambda x, bold, hrf: MSE(x, bold, hrf))(x, bold, hrf).ravel()
#
# def fun_hess(x, bold, hrf):
#   return Hessian(lambda x, bold, hrf: MSE(x, bold, hrf))(x, bold, hrf)
...
def estimateAll(bolds, hrf, margin = 0):
  ...
  plt.grid(None)
  plt.title('$x$ estimation error percentages with average of %.3f'%Xs_err_mean)
  ...
  plt.grid(None)
  plt.title('$y$ estimation error percentages with average of %.3f'%Ys_err_mean)
  sns.heatmap(img, square=True, cmap="YlGnBu", vmin=0, vmax=5)
  plt.show()
  ...
  plt.grid(None)
  plt.title('$\sigma$ estimation error percentages with average of %.3f'%Ss_err_mean)
  sns.heatmap(img, square=True, cmap="YlGnBu", vmin=0, vmax=5)
  plt.show()
